...
export const PricingCard: React.FC<PricingCardProps> = ({ 
  title, price, description, features, isPopular, colorType = 'web' 
}) => {
  ...
  return (
    <div className={cn(
      ...
      {isPopular && (
        <div className={cn("absolute top-0 right-0 text-white text-xs font-bold px-3 py-1 rounded-bl-lg", colors.badge)}>
          POPULAR
        </div>
      )}
      ...
      <div className="p-8 flex-grow flex flex-col">
         <div className="text-center mb-6">
            <span className={cn("text-4xl font-extrabold", colors.text)}>{price}</span>
         ...
         <ul className="space-y-4 mb-8 flex-grow">
           {features.map((feature, idx) => (
             <li key={idx} className="flex items-start gap-3 text-sm text-slate-600">
               <Check size={16} className={cn("mt-0.5 flex-shrink-0", colors.text)} />
               <span>{feature}</span>
             </li>
           ))}
         </ul>
         ...
         <button 
           className={cn(
             ...
             isPopular ? colors.button : "bg-slate-800 hover:bg-slate-700"
           )}
         >
           Seleccionar plan
         </button>
